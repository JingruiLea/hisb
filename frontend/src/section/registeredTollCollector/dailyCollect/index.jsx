...
class DailyCollectSection extends React.Component {
  ...
  columns = [{
    ...
    title:"操作",
    render:(data)=>(<Button onClick={()=>{this.viewHistoryDailyCollectDetail(data)}}>查看</Button>)
  }]
  ...
  viewHistoryDailyCollectDetail=(reqData)=>{
    API.request(API.outpatientWorkstation.dailyCollect.dailyCollectDetail,{daily_collect_id:reqData.daily_collect_id})
    .ok((list)=>{
      this.setState({
        historyTableData:{
          list:list,
  ...
  submitDailyCollect=(reqData)=>{
    ...
    .ok((data)=>{
      ...
      this.viewHistoryDailyCollectDetail({
        daily_collect_id:data.id,
        start_time:reqData.start_time,
        end_time:reqData.end_time,
      })
    }).internalError(data=>{
      Message.openNotification('错误','日结失败，请检查日期，不要重复结算');
    }).submit();
  }
        ...
        <Col span={16}>
          <Card title={"历史日结详情"} style={{minHeight:'850px'}}>
            <span style={{marginRight:'100px'}}>
              <b>开始时间：{state.historyTableData.start_time}</b> 
            </span>
            <span>
              <b>结束时间:{state.historyTableData.end_time}</b>
            </span><br/><br/>
